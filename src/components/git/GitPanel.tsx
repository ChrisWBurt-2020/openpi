...
export function GitPanel(props: GitPanelProps) {
  ...
  const handleFileClick = async (file: GitChangedFile) => {
    ...
    try {
      const scope = diffScope() === 'branch' ? 'branch' : diffScope() === 'staged' ? 'staged' : 'unstaged'
      const diff = await window.openpi.git.getDiff(file.path, props.cwd ?? undefined, {
        scope,
      })
      console.log('[openpi:git] handleFileClick diff=%s (path=%s, cwd=%s, scope=%s)', diff ? 'ok' : 'null', file.path, props.cwd, scope)
      if (diff && mounted) {
        const idx = current.files.indexOf(file)
        props.onDiffOpen(diff, current.files, idx)
      }
    } catch (err) {
      console.warn('[openpi:git] handleFileClick error (path=%s): %s', file.path, (err as Error).message)
    } finally {

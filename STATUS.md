...
- **Workspace stabilization (unreleased):** local non-Git folders retain their filesystem tree without Git metadata; Pi event/statistics failures are isolated from the conversation; SSH Workspace transport has validated requests, bounded deadlines, SFTP reuse, cancellation, and diagnostic correlation.

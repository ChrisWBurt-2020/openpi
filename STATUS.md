...
- **Durable Runs (experimental):** Ask/Run intent, V2 persisted state/event records, settlement-gated continuations, worker-loss lease reconciliation, queued checkout conflicts, local/Persistent Runner control tools, status/input/pause cards, and bounded tool evidence are staged. Full SSH smoke coverage, managed-worktree conflict UX, authoritative Git/diff receipts, runner lease/replay protocol, and notification routing remain incomplete. Ask is the supported path while the workspace stabilization gate is open.

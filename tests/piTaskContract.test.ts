...
/**
 * Contract tests for OpenPi's alignment with `@heyhuynhgiabuu/pi-task` v0.2.0.
 * These verify the rules we enforce in OpenPi match what pi-task actually does.
 */

describe('pi-task v0.2.0 contract', () => {
  it('fresh task mints a short base36-timestamp-short id (not UUID)', () => {
    // pi-task v0.2.0 src/index.ts: id = ${Date.now().toString(36)}-${randomUUID().slice(0,4)}
  ...
  it('fresh task must omit task_id and conversation_id', () => {
    // Per pi-task v0.2.0 execute(): if registeredTaskId -> resume via conversation_id;
  ...
  it('background is the default in pi-task v0.2.0', () => {

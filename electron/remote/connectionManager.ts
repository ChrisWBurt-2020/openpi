...
export class RemoteConnectionManager {
  ...
  async workspaceOperation(
    ...
    const toRemote = async (value: string, write = false): Promise<string> => {
      ...
      const resolved = await this.sftpRealpath(sftp, checkPath)
    ...
    switch (request.operation) {
      ...
      case 'find': {
        const target = await toRemote(this.requireWorkspacePath(request))
        const output = await this.execWithEnv(
          client,
          'cd -- "$OPENPI_WORKSPACE" && find . -type f -name "$OPENPI_PATTERN" -print',
          {
            OPENPI_WORKSPACE: target,
            OPENPI_PATTERN: request.pattern ?? '*',
          }
        )
        return output
          .split(/\r?\n/)
  ...
  private execWorkspaceShell(
    ...
    return new Promise((resolve, reject) => {
      client.exec(
        'cd -- "$OPENPI_WORKSPACE" && exec sh -s',
        { env: { OPENPI_WORKSPACE: cwd } },
        (error, stream) => {
          if (error) return reject(error)
          let output = ''
          let timedOut = false

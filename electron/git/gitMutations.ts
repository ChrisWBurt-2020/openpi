 ...
 *
 * All operations are serialized through withGitLock to prevent index.lock
 * conflicts when multiple operations fire concurrently.
 */
...
/**
 * Write a hunk patch to a temp file and run `git apply` with the given flags.
 * Returns stdout/stderr on success, throws on failure.
 */
async function applyHunkPatch(
  cwd: string,
  patchText: string,
  extraArgs: string[] = []
): Promise<string> {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'openpi-hunk-'))
  const tmpFile = path.join(tmpDir, 'hunk.patch')
  fs.writeFileSync(tmpFile, patchText, 'utf-8')
  try {
    const git = simpleGit({ baseDir: cwd })
    return await git.raw(['apply', ...extraArgs, tmpFile])
  } finally {
    fs.rmSync(tmpDir, { recursive: true, force: true })
  }
}

/** Stage a single hunk: `git apply --cached <hunk-patch>` */
export async function stageHunk(
  ...
  return withGitLock(cwd, async () => {
    try {
      const output = await applyHunkPatch(cwd, hunkPatch, ['--cached'])
      return { ok: true, filePath, output: output.trim() || `Staged hunk in ${filePath}.` }
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error)
      return { ok: false, filePath, output: msg }
    }
  })
}

/** Unstage a single hunk: `git apply --cached --reverse <hunk-patch>` */
export async function unstageHunk(
  ...
  return withGitLock(cwd, async () => {
    try {
      const output = await applyHunkPatch(cwd, hunkPatch, ['--cached', '--reverse'])
      return { ok: true, filePath, output: output.trim() || `Unstaged hunk in ${filePath}.` }
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error)
      return { ok: false, filePath, output: msg }
    }
...
/** Revert a single hunk from the working tree: `git apply --reverse <hunk-patch>` */
export async function revertHunk(
  ...
  return withGitLock(cwd, async () => {
    try {
      const output = await applyHunkPatch(cwd, hunkPatch, ['--reverse'])
      return { ok: true, filePath, output: output.trim() || `Reverted hunk in ${filePath}.` }
    } catch (error) {
      const msg = error instanceof Error ? error.message : String(error)
      return { ok: false, filePath, output: msg }
    }
